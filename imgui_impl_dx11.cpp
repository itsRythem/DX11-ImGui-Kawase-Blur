...
bool ImGui_ImplDX11_LoadTexture(void* texture_data, size_t size, ID3D11ShaderResourceView** texture, ID3D11Device* device)
{
    ImGui_ImplDX11_Data* bd = ImGui_ImplDX11_GetBackendData();
    if (bd && !device)
        device = bd->pd3dDevice;

    D3DX11_IMAGE_LOAD_INFO info;

    ID3DX11ThreadPump* pump{ nullptr };
    if (FAILED(D3DX11CreateShaderResourceViewFromMemory(device, texture_data, size, &info, pump, texture, NULL)))
        return false;
